...
if [ $USER == 'root' ]; then
	 echo Please logon as a user to execute the installation,  not root
	 exit 1
fi
...
outputLog() {
    echo -e "\e[96m$1\e[90m" | tee -a $logfile
}
outputResponse() {
    echo $1 >> $logfile
}
...
outputLog "Updating packages ..."
upgrade=$false
update=$(sudo apt-get update 2>&1)
#echo $update >> $logfile
outputResponse $update
update_rc=$?
...
#		echo -e "\e[93mIt seems like LusionMagic is already installed." | tee -a $logfile
#		echo -e "To prevent overwriting, the installer will be aborted." | tee -a $logfile
#		echo -e "Please rename the \e[1m~/LusionMagic\e[0m\e[93m folder and try again.\e[0m" | tee -a $logfile
#		echo ""
#		echo -e "If you want to upgrade your installation run \e[1m\e[97mupgrade-script\e[0m from the ~/LusionMagic/installers directory." | tee -a $logfile
#		echo ""
#		exit;
#	fi
#
#	echo -e "\e[96mCloning LusionMagic ...\e[90m" | tee -a $logfile
#	if git clone --depth=1 https://github.com/robsecord/LusionMagic.git; then
#		echo -e "\e[92mCloning LusionMagic Done!\e[0m" | tee -a $logfile
#	else
#		echo -e "\e[91mUnable to clone LusionMagic." | tee -a $logfile
#		exit;
#	fi
#
#	cd ~/LusionMagic  || exit
...
#					echo -e "\e[92mSplashscreen: Changed theme to LusionMagic successfully.\e[0m" | tee -a $logfile
#				else
#					echo -e "\e[91mSplashscreen: Couldn't change theme to LusionMagic!\e[0m" | tee -a $logfile
#				fi
#			fi
#		else
...
#fi
echo -e "\e[92mWe're ready! Run \e[1m\e[97m$rmessage\e[0m\e[92m from the ~/LusionMagic directory to start your LusionMagic.\e[0m" | tee -a $logfile
